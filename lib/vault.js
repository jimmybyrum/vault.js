...
var Vault = (function() {
  ...
  var prepare = function(value, encode) {
    ...
    if (encode && typeof value === 'object' && window.JSON !== undefined && mode === 'browser') {
  ...
  var setup = function(type) {
    ...
    return {
      getData: function() {
        var vaultDataDictionary = storage.getItem(config.vaultData);
      ...
      setKeyMeta: function(key, config) {
        if (config) {
          ...
          storage.setItem(config.vaultData, prepare(vaultDataDictionary));
        }
      },
      ...
      set: function(key, value, config) {
        try {
          ...
          return storage.setItem(key, prepare(value, false));
        } catch(e) {
  ...
  if (mode === 'server') {
    ...
    var _file = appDir + config.vaultFile;
    console.warn(_file);
    var cache = {};
    ...
    var Server = {
      save: function(callback) {
        fs.writeFile(_file, JSON.stringify(cache), function(err) {
          if (err) {
            console.error(err);
          }
